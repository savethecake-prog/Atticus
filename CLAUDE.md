...
- Identifiers come from the distributor's master data (the source of truth), not from web-scraping variant barcodes, which is exactly where a wrong barcode lands on the wrong variant. Model number can be derived from the product title where the title states it, flagged as title-derived for confirmation. The manufacturer product page's structured `gtin13` is a manufacturer-tier EAN source and is preferred over a retailer-mirror barcode; its `og:` width/height/depth and weight give per-variant packaging when the datasheet omits them.
- The instrument matters (Thermal Grizzly, 2026-06-11). In a sandboxed/proxied environment WebFetch and curl are often host-blocked (403), and WebSearch returns a model's *summary* of a page, not the page — those summaries are unsignable and were proven wrong here (an invented Kryonaut "12.5 W/mK" and a KryoSheet temperature absent from the real table). Source verbatim with a server-side scraper (Firecrawl) that returns the actual PDF/HTML; never sign a WebSearch summary. When a datasheet's spec table is a baked image that will not extract, fall back to the product page HTML (and a sister-brand shop if one exists), and use JSON extraction to pull the table cleanly.
- A field absent across an entire brand can be deliberate policy, not a gap: Thermal Grizzly publishes no numeric thermal conductivity (W/mK) for any product (its FAQ states this, since Q4 2020), and no manufacturer warranty period. Cite the policy page as the receipt and treat the blank as a sourced negative, not an unknown — but only after the search that proves it.
- A PDF table whose column order is garbled in extraction (net/gross swapped) is disambiguated by physics (gross ≥ net) and flagged for confirmation, never guessed.
...
- Schema conformance (`schema_spec.py`): every cell that maps to a field in Christopher's structured schema (`references/schema/schema_rules.json`) satisfies that field's stated rules — char-length, single-select set, compaction (no internal spaces), required glyph (⎓, ²), banned token (MT/s), and the U+0027 apostrophe in What's-in-the-box. The validator keys on the field, not a header substring, and only judges cells it can confidently map; an unmapped header carries no invented rule. It reports, it never rewrites.
...
- Not-applicable is not a gap (Thermal Grizzly, 2026-06-11). When several product classes share one tab on a union column set, most blanks are a column that does not apply to that row — viscosity on a spatula, thickness on a paste — and are correct; SALT drops empty not-applicable fields. A blank is only a gap when the field is applicable to that product class and still empty. The client gap document must be applicability-aware: list per product the applicable-but-unfound cells with a reason and a source (distributor master, vendor chase, weigh), and exclude the not-applicable blanks, or the client drowns in noise. Where the union is wide, per-category tabs read cleaner than one sheet full of legitimately-empty cells.
